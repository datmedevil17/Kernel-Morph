...
const deployToTestnet = async (
  ...
  try {
    ...
    logger(`❌ Testnet deployment error: ${error}`, "error")
    return false
  }
}
...
const deployToMainnet = async (
  ...
  if (!isConnected) {
    logger(`❌ Wallet not connected. Mainnet deployment requires wallet connection.`, "error")
    return false
  }

  if (!compilationResult) {
    logger(`❌ No compilation result found. Mainnet deployment requires compilation.`, "error")
    return false
  }
  ...
  const confirmed = window.confirm(
    ...
    `You are about to deploy to Asset Hub Mainnet.\n` +
    `Network: ${node.config.network}\n` +
    `Gas Limit: ${node.config.gasLimit}\n` +
    `Gas Price: ${node.config.gasPrice}\n\n` +
    `This will use real funds. Are you sure you want to proceed?`
  )

  if (!confirmed) {
    logger(`❌ Mainnet deployment cancelled by user`, "error")
    return false
  }

  try {
    logger(`🔄 Processing mainnet deployment...`, "info")
    ...
    logger(`✅ Mainnet deployment simulation completed`, "success")
    ...
    logger(`❌ Mainnet deployment error: ${error}`, "error")
    return false
  }
}
...
const handleInputChange = (key: string, value: string | number | boolean) => { // Changed from any
  ...
  <>
    <div>
      ...
      {node.type === "deployMainnet" && (
        <p className="text-xs text-red-400 mt-1">⚠️ Mainnet deployment - use with caution</p>
      )}
    </div>
    <div>
 ...
 const renderWalletButton = () => {
  ...
  <div className="p-4 border-b border-gray-800 flex-shrink-0 mt-26">
    <h3 className="text-lg font-semibold text-white">Solidity Pipeline</h3>
  </div>
